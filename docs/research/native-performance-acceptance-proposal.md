...
The following continuation adds optional [declared schedule accounting](native-performance-observation-format.md#optional-declared-schedule) to that same offline reader: phase-preserving rational opportunities, explicit skips or render links, separate unscheduled work, and closed-cohort resolution. Publication/selection/submission/disposition timestamps expose published-frame residence and age, including drops/errors, without resetting them on replay. This is diagnostic arithmetic over supplied JSON, not authenticated schedule predeclaration, queue-capacity proof, physical refresh counting or approved deadlines. Legacy captures remain schedule-unknown; no native deadline or release verdict is inferred.
